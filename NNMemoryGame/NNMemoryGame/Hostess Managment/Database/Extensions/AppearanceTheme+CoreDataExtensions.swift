...
extension AppearanceTheme {
    ...
    static func themeFromJSONDicitonary(themeDict: JSON) -> AppearanceTheme {
        ...
        if themeDict["roulette_border_color"].stringValue.count > 0 {
            theme.carouselBorderColor = UIColor.hexStringToUIColor(hex: themeDict["roulette_border_color"].stringValue)
        } else {
            theme.carouselBorderColor = UIColor.red
        }
